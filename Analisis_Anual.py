...
def app():
   #set page
    ...
    st.title(':clipboard: Análisis ANUAL de Resultados') #Titulo del Dash
    ...
    def load_data(file):
        if file == "Excel":
            df_t = pd.read_excel(file, encoding="utf8", errors='ignore')
        else:
    ...
    ################################################################
    df_t['materias_aprobadas'] = df_t['Nota'].apply(contar_aprobadas_y_no_aprobadas)

    # Añadir una nueva columna con el nombre del curso
    df_t['Nivel'] = df_t['Grupo_Clase'].apply(identificar_curso)

    st.dataframe(df_t)

    ############# Cáculo del número aprobados - supendidos - porcentajes por materia/grupo/evaluación ########

    # Filtrar las materias aprobadas (Nota igual o superior a 5)
    df_aprobadas = df_t[df_t['Nota'] >= 5]

    # Calcular el número de materias aprobadas por cada materia, evaluación y nivel
    df_num_aprobadas = df_aprobadas.groupby(['Grupo_Clase', 'Materia', 'Evaluacion', 'Nivel']).agg({'Nota': 'count'}).reset_index()
    df_num_aprobadas.rename(columns={'Nota': 'Num_aprobadas'}, inplace=True)

    # Calcular el número total de materias por cada materia, evaluación y nivel
    df_total_materias = df_t.groupby(['Grupo_Clase', 'Materia', 'Evaluacion', 'Nivel']).agg({'Nota': 'count'}).reset_index()
    df_total_materias.rename(columns={'Nota': 'Total_materias'}, inplace=True)

    # Combinar los dos DataFrames para calcular el porcentaje de aprobados por cada materia, evaluación y nivel
    df_porcentaje_aprobados = pd.merge(df_num_aprobadas, df_total_materias, on=['Grupo_Clase', 'Materia', 'Evaluacion', 'Nivel'])
    df_porcentaje_aprobados['Porcentaje_aprobados'] = (df_porcentaje_aprobados['Num_aprobadas'] / df_porcentaje_aprobados['Total_materias']) * 100

    # Obtener lista de grupos únicos
    grupos = df_porcentaje_aprobados['Grupo_Clase'].unique()

    # Widget para seleccionar el grupo
    selected_group = st.selectbox('Seleccionar grupo:', grupos)

    # Filtrar DataFrame por el grupo seleccionado
    df_selected_group = df_porcentaje_aprobados[df_porcentaje_aprobados['Grupo_Clase'] == selected_group]

    # Filtrar DataFrame por la evaluación deseada (1EVA, 2EVA, ORD)
    df_selected_eval = df_selected_group[df_selected_group['Evaluacion'].isin(['1EVA', '2EVA', 'ORD', 'Ev. Ord. 2º - En condiciones de titular','Ev. Ext. 2º - En condiciones de titular','Ev. Ord. 1º - Alumnado de 2º sin condiciones de titular','Ev. Ext. 1º - Alumnado de 2º sin condiciones de titular','Ev. Ord. 2º - Sin condiciones de titular','Ev. Ord. 1º - Alumnado de 2º en condiciones de titular','Ev. Ext. 1º - Alumnado de 2º en condiciones de titular','Ev. Ext. 2º - Sin condiciones de titular','Convocatoria ordinaria'])]

    # Crear el gráfico de barras con Altair
    chart = alt.Chart(df_selected_eval).mark_bar().encode(
        x='Materia',
        y='Porcentaje_aprobados',
        color='Evaluacion:N',
        tooltip=['Materia', 'Porcentaje_aprobados']
    ).properties(
        width=600,
        height=400,
        title=f'Comparativa ANUAL del PORCENTAJE de APROBADOS por MATERIA para el grupo {selected_group}'
    )

    # Mostrar el gráfico utilizando Streamlit
    st.altair_chart(chart, use_container_width=True)

    st.dataframe(df_selected_eval)
